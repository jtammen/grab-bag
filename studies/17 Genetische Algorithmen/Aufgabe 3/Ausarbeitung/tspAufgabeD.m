...
% Für Aufgabe d), je 20 Testläufe
subpopulations = 12;
individuals 	= 60;
numTestRuns 	= 20;

% Aufgabe d)
selections = {'selsus','selrws','seltour'};
...
for actSelect = 1 : 3
   ...
   for run = 1 : numTestRuns
      ...
      GeaOpt = geaoptset( GeaOpt ...
         ...
         ...
         , 'Selection.Name',             	selections(actSelect)...% Selektion: selsus, selrws, seltour
         , 'Recombination.Name',         	'recgp'  ... 				% Rekombination: recgp, recpm, reccycle (selbst implementieren!) 
         , 'Mutation.Rate',              	0.9      ... 				% Mutation: Rate
         , 'Mutation.Name', 				  	{'mutswap', 'mutmove', 'mutinvert'} ... % Mutation: Verfahren
         );
      
      % Speichern der Ergebnisse
      GeaOpt = geaoptset( GeaOpt ...
         , 'Output.SaveTextInterval',    20 ...        				% Text to File every xx generations
         , 'Output.SaveTextFilename',    'logging.log' ... 			% Filename of result file, absolut or relative path may be included
         );
