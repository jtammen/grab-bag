...
%clear ySim tSim errors success totalSuccess;

% Simulation mit _allen_ Datensaetzen
% tSimClass1 = repmat([1; 0; 0], 1, length(class1Samples));
% tSimClass2 = repmat([0; 1; 0], 1, length(class2Samples));
% tSimClass3 = repmat([0; 0; 1], 1, length(class3Samples));
% % In tSim stehen die SOLL-Werte
% tSim = horzcat(tSimClass1, tSimClass2, tSimClass3);
% sortedThyroid = [class1Samples(:,1:21); class2Samples(:,1:21); class3Samples(:,1:21)];
% [ySim, pf, af, eSim, perf] = sim(net, sortedThyroid');
% 
% [errors, success, totalSuccess] = calculatePerformance(ySim, tSim, ...
%     {class1Samples(:,1:21); class2Samples(:,1:21); class3Samples(:,1:21)});
% 
% disp('Simulation mit Komplettmenge abgeschlossen');
% disp(sprintf('Fehler: C1: %d/%d (OK: %f), C2: %d/%d (OK: %f), C3: %d/%d (OK: %f)',...
%     errors(1), length(class1Samples(:,1:21)), success(1),...
%     errors(2), length(class2Samples(:,1:21)), success(2),...
%     errors(3), length(class3Samples(:,1:21)), success(3)));
% disp(sprintf('Klassifikationsguete gesamt: %f', totalSuccess));