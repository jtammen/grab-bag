...
% Datum: 13.06.2005
        ...
        /*** Syntaktische Regeln ***/
        sentence                		-->     nounPhrase( nominative, Number, Person ),
                                        		verbPhrase( Number, Person ).

        nounPhrase( Case, Number, 3      )      -->     determiner( Gender, Number, Case ),
                                                        noun( Gender, Number, Case ).
        % Neue Regel: NP -> PP
        nounPhrase( Case, Number, Person )      -->     personalPronoun( Number, Person, Case ).


        verbPhrase( Number, Person )            -->     verbTransitive( Number, Person ),
                                                        nounPhrase( accusative, _, _ ).
        verbPhrase( Number, Person )            -->     verbIntransitive( Number, Person ).
